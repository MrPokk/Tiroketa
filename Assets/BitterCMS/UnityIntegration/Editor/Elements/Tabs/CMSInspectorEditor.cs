...
{
    public class CMSInspectorEditor : CMSEditorTab
    {
        ...
        {
            if (Info.DeserializedEntity == null)
            {
                ShowErrorMessage();
                return;
            }
            ...
            {
                DrawFields.DrawFieldsForObject(Info.DeserializedEntity);
            }
        ...
        {
            if (!GUILayout.Button("Save") || Info.DeserializedEntity == null)
                return;
        ...
        {
            if (!GUILayout.Button("Refresh") || Info.DeserializedEntity == null)
                return;
        ...
        {
            var allSerializeComponents = Info.DeserializedEntity.GetSerializeComponents();
        ...
        {
            if (Activator.CreateInstance(Info.DeserializedEntity.ID) is not CMSEntity newEntity)
                return;

            var allComponent = Info.DeserializedEntity.GetSerializeComponents();
            foreach (var component in allComponent)
            {
                var idComponent = component.ID;
                if (!newEntity.HasComponent(idComponent))
                    Info.DeserializedEntity.RemoveComponent(idComponent);
            }
        }
