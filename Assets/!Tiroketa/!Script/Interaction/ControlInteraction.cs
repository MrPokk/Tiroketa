...
namespace Game._Script.Interaction
{
    public class ControlInteraction : BaseInteraction, IEnterInStart, IExitInGame, IEnterInUpdate
    {
        ...
        private void InteractionMove()
        {
            ...
            {
                entity.GetComponent(out ControlComponent controlComponent).ReadInput = inputVector2;
            }
        }

        private void InteractionAttack(InputAction.CallbackContext context)
        {
            var allModel = CMSRuntimer.GetPresenter<MobPresenter>().GetEntitiesToComponent(
                typeof(ControlComponent), typeof(AttackProcessComponent));
            
            foreach (var entity in allModel)
            {
                entity.GetComponent(out AttackProcessComponent _).Properties.AttackProcess.Invoke();
            }
        }
